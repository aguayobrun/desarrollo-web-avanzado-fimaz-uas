...
class ProductoModel {
    ...
    public function actualizar (int $id, array $data) : bool {
        try {
            $this -> conexion -> beginTransaction();
            $sql = "UPDATE productos SET sku = :sku, nombre = :nombre, descripcion = :descripcion, 
            precio_compra = :precio_compra, precio_venta = :precio_venta, existencia = :existencia WHERE id = :id";
            $stmt = $this -> conexion -> prepare ($sql);
            $stmt -> bindParam(':sku', $data['sku']);
            $stmt -> bindParam(':nombre', $data['nombre']);
            $stmt -> bindParam(':descripcion', $data['descripcion']);
...
/* la clase de abajo es una clase de modelo de PHP para interactuar con una tabla de base de datos llamada 'productos' para
realizar operaciones como obtener todos los registros, búsqueda, creación, actualización y eliminación de registros. */
?>